...
public class AutorizzazioniManager {
    ...
    private Autorizzazione parseRiga(String riga) {
        ...
        int numero;
        try {
            numero = Integer.parseInt(campi[0].trim());
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Numero autorizzazione non numerico: \"" + campi[0] + "\"");
        }

        String scadenza        = campi[1].trim();
        String richiedente     = campi[2].trim();
        String posizione       = campi[3].trim();
        String tipoMezzo       = campi[4].trim();
